...
[핵심 결과](#핵심-결과) · [분석 흐름](#분석에서-운영안까지) · [운영안](#셔틀-운영안) · [한계](#해석-범위)
...
여의도 불꽃축제가 끝나는 시점에는 많은 관람객이 짧은 시간에 이동하면서 지하철역과 버스 정류장에 수요가 집중됩니다. 이 프로젝트는 OD 이동량, 생활·체류인구, 버스 운행과 GIS 데이터를 같은 시간·공간 단위로 결합해 혼잡이 발생하는 조건과 이동 방향을 분석한 졸업 연구입니다.

분석 결과를 상관계수나 지연 추정치로 끝내지 않고 셔틀 운영 시나리오로 옮겼습니다. 장거리 목적지별 노선을 만드는 대신 공덕·당산·노량진 환승거점으로 분산하고, 좌석 수와 차량 수, 회전수로 수송 용량을 계산했습니다. 비용과 현장 제약을 포함하되, 관측자료 기반 시나리오를 인과효과나 확정 운영안으로 해석하지 않았습니다.

### 시작한 이유

교통 데이터 분석이 의사결정에 쓰이려면 "혼잡하다"는 설명에서 멈추지 않고 언제, 어디에, 어느 규모로 공급을 넣을지 답해야 합니다. 여러 데이터의 관측 범위와 한계를 유지하면서 실행 가능한 대안을 만드는 과정을 보여주기 위해 이 주제를 선택했습니다.
