![Major-Event Traffic Delay project hero](assets/project-hero.svg)

<div align="center">

**행사 종료 후 수요 집중을 여러 교통 데이터로 분석하고, 3개 환승거점 셔틀 운영안으로 연결한 졸업 연구 case study**

![Analysis](https://img.shields.io/badge/Analysis-Python%20%2B%20GIS-3776AB)
![Data](https://img.shields.io/badge/Data-OD%20%2B%20Bus%20%2B%20Population-0F766E)
![Domain](https://img.shields.io/badge/Domain-Transportation-D97706)
![Scope](https://img.shields.io/badge/Public-Decision%20Case%20Study-7C3AED)

[분석 흐름](#분석-흐름) · [관측 결과](#관측-결과) · [운영 시나리오](#운영-시나리오) · [해석 범위](#해석-범위)

</div>

---

> 이 저장소는 문제 정의, 집계 결과와 운영 시나리오를 공개한 case study입니다. 이용 조건과 크기 제약이 있는 원천 데이터와 전체 분석 코드는 포함하지 않습니다.

## 의사결정 제약

여의도 불꽃축제가 끝나면 많은 관람객이 짧은 시간에 이동해 역과 정류장에 수요가 집중됩니다. 혼잡 수치만 제시해서는 실제 운영안을 정하기 어렵습니다. 언제 수요가 몰리는지, 어느 방향으로 분산할지, 어느 정도의 차량이 필요한지를 함께 설명해야 합니다.

## 분석 질문

> 서로 다른 시간·공간 단위의 교통 데이터를 결합해 행사 종료 후 수요 집중을 설명하고, 실행 가능한 공급 규모와 환승거점 시나리오로 바꿀 수 있는가?

이 연구는 셔틀의 인과적 효과를 증명하는 실험이 아닙니다. 관측 데이터에서 수요와 지체의 관계를 정리하고, 현장 검토를 시작할 수 있는 운영 규모를 산정한 case study입니다.

## 접근과 선택 이유

OD 이동량, 생활·체류인구, 버스 운행과 GIS 데이터를 같은 시간·공간 단위로 맞췄습니다. 관측된 관계는 지체 추정, 환승거점 선정, 수송 용량과 개략 비용 계산으로 이어졌습니다.

### 왜 여러 데이터를 결합했는가

OD만 보면 이동 방향은 알 수 있지만 행사장 주변에 사람이 언제 쌓이는지 설명하기 어렵습니다. 생활·체류인구는 시간대별 집중을, 버스 자료는 공급과 지연의 관계를, GIS는 환승 가능한 공간 조건을 보여줍니다. 각 데이터가 답할 수 있는 질문을 나누고 같은 시간·공간 단위로 맞췄습니다.

### 왜 isotonic regression을 사용했는가

초과 교통량이 늘어났는데 예측 지체시간이 오히려 내려가는 결과는 이 분석의 교통공학적 가정과 맞지 않습니다. 다만 증가 폭이 항상 일정하다고 볼 근거도 부족했습니다. 그래서 직선의 고정 기울기를 강제하는 대신, 초과 교통량이 증가할수록 예측 지체가 감소하지 않는 단조 관계만 제약하는 isotonic regression을 사용했습니다.

이 선택은 초과 교통량이 지체를 인과적으로 발생시킨다는 증명이 아닙니다. 관측 범위 안에서 관계의 방향을 유지한 시나리오 추정이며, 범위를 벗어난 외삽이나 실제 운영 효과 예측에는 사용하지 않았습니다.

### 왜 최종 목적지가 아니라 환승거점을 선택했는가

모든 귀가 목적지로 노선을 만들면 노선 수가 많아지고 차량 회전이 느려집니다. 행사장에서 가까우면서 서로 다른 철도 방향으로 전환할 수 있는 공덕·당산·노량진까지만 연결해, 짧은 회전으로 수요를 분산하는 시나리오를 만들었습니다.

### 왜 비용과 수송 용량을 함께 계산했는가

상관계수와 지연 추정치만으로는 운영 규모를 결정할 수 없습니다. 좌석 수, 차량 수와 회전수를 곱해 약 13,000명의 추가 수요와 비교하고, 개략 비용도 함께 계산했습니다. 이는 확정 운영안이나 효과 검증이 아니라 현장 검토를 시작하기 위한 규모 산정입니다.

## 분석 흐름

```mermaid
flowchart LR
    A["OD movement"] --> J["Time-space join"]
    B["Living population"] --> J
    C["Bus operations"] --> J
    D["GIS network"] --> J
    E["Event constraints"] --> J
    J --> F["EDA and correlation"]
    F --> G["Demand and delay<br/>scenario"]
    G --> H["Hub screening"]
    H --> I["Capacity and cost<br/>concept"]
```

| 데이터 | 사용 목적 |
|---|---|
| OD 이동량 | 행사 전후 유입·유출 방향 비교 |
| 생활·체류인구 | 수요가 집중되는 시간대 확인 |
| 버스 운행 | 공급과 지연의 관계 확인 |
| GIS와 운영 조건 | 환승거점과 현장 제약 검토 |

## 관측 결과

변수 조합에서 확인한 주요 상관계수는 `0.5864`, `0.5969`, `0.7034`였습니다. 이 값은 변수 간 관계를 보여주며 셔틀의 인과적 효과를 뜻하지 않습니다.

초과 교통량과 지체시간의 단조 관계를 적용해 약 13,000명의 추가 수요를 가정한 시나리오에서 평균 지연을 추정했습니다.

| 혼잡 가정 | 평균 지연 추정 |
|---|---:|
| 낮음 | **0.80분** |
| 기준 | **1.26분** |
| 높음 | **1.93분** |

## 운영 시나리오

여의도에서 최종 목적지까지 직접 수송하지 않고 철도·간선 교통으로 전환할 수 있는 세 거점까지 짧게 연결합니다.

| 거점 | 분산 방향 |
|---|---|
| 공덕 | 서북권·공항철도·도심 |
| 당산 | 서부권·2호선·9호선 |
| 노량진 | 남부권·1호선·9호선 |

```text
45 seats × 100 buses × 3 rotations = 13,500 passenger movements
```

약 5천만원은 차량 규모를 설명하기 위한 개략 비용입니다. 기사와 통제 인력, 승하차 공간, 회송 거리와 조달 방식은 별도 검토가 필요합니다.

## 해석 범위

- 관측 상관은 셔틀의 인과적 개선효과가 아닙니다.
- 평균 지연 추정은 미시 교통 simulation이나 현장 실험을 대체하지 않습니다.
- 기준일과 공간 단위에 따라 결과가 달라질 수 있습니다.
- 비용은 견적이 아니라 규모를 비교하기 위한 추정치입니다.
- 실제 적용 전 민감도 분석, 도로 통제, 승하차 공간과 운영기관 협의가 필요합니다.

## 기여

교통공학 관점의 연구 질문, 데이터 선택과 결합, EDA, scenario assumption, 환승거점·수송규모·비용 산정과 해석 범위를 설계했습니다.

[미구현 데이터·분석 확장 계획](docs/LEARNING_ROADMAP.md)
