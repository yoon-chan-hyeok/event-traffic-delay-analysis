...
먼저 행사 종료 뒤 혼잡이 언제, 어느 방향에 집중되는지 선행 사례와 가용 데이터를 기준으로 문제를 나눴습니다. OD, 생활인구, 버스와 GIS 자료를 같은 시간과 공간 단위로 맞춘 가벼운 분석으로 수요 집중 가설을 확인한 다음, 거점과 수송 용량을 포함한 운영 시나리오로 확장했습니다.

결과가 실제 의사결정에 쓰이는 상황을 생각해 좌석 수, 차량 수, 회전수와 비용을 함께 계산했습니다. 동시에 관측 상관을 인과효과로 읽지 않도록 해석 범위를 분리했습니다. 기준일과 공간 단위에 따른 민감도, 준실험 설계와 scenario calculator는 다음 검증 단계로 남겼습니다.
